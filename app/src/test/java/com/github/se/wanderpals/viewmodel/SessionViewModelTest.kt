...
class SessionViewModelTest {
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `updateUserForCurrentUser updates user role successfully`() = runBlockingTest(testDispatcher) {
        val tripId = "trip1"

        viewModel.updateUserForCurrentUser(tripId)
        advanceUntilIdle()
        assertEquals(Role.OWNER, SessionManager.getCurrentUser()?.role) // Check if the role is updated
    }

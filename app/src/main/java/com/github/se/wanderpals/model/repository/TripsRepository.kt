...
open class TripsRepository(
  ...
  open suspend fun updateSuggestionInTrip(tripId: String, suggestion: Suggestion): Boolean =if (checkNetworkIsValidAndLog())
     ...
     */
  open suspend fun getUserFromTrip(tripId: String, userId: String): User? = userRepository.getUserFromTrip(tripId = tripId, userId = userId, source = getSource())
   ...
   *   an error.
   */
  open suspend fun getAllUsersFromTrip(tripId: String): List<User> =userRepository.getAllUsersFromTrip(tripId = tripId, source = getSource())
   ...
   */
  open suspend fun addUserToTrip(tripId: String, user: User): Boolean =if (checkNetworkIsValidAndLog())
      userRepository.addUserToTrip(tripId = tripId, user = user)
  else false
   ...
   * @return `true` if the operation is successful, `false` otherwise.
   */
  open suspend fun updateUserInTrip(tripId: String, user: User): Boolean =if (checkNetworkIsValidAndLog())
      userRepository.updateUserInTrip(tripId = tripId, user = user)
  else false
   ...
   * @return `true` if the operation is successful, `false` otherwise.
   */
  open suspend fun removeUserFromTrip(tripId: String, userId: String): Boolean =if (checkNetworkIsValidAndLog())

                ...
                ElevatedCard(
                    ...
                    Column(modifier = Modifier.background(MaterialTheme.colorScheme.surface)) {
                        ...
                        Column {
                            for (key in users) {
                            DebtInfo(
                                amount = debt[key.userId]!!.values.sumOf { it } - debt[key.userId]!![key.userId]!!,
                                user = key.name
                                )
                            }
                        }
                ...
                ElevatedCard(
                    ...
                    Column(modifier = Modifier.background(MaterialTheme.colorScheme.surface)) {
                        ...
                        Spacer(modifier = Modifier.height(8.dp))
                                ...
                                Column {
                                    users.filter { it.userId != SessionManager.getCurrentUser()!!.userId }
                                        .forEach { key ->
                                            val a : MutableMap<String, Double>? = debt.get(SessionManager.getCurrentUser()!!.userId)
                                            val b = a?.get(key.userId)
                                            val c = SessionManager.getCurrentUser()

                                            if(debt[SessionManager.getCurrentUser()!!.userId]!![key.userId]!! != 0.0) {
                                                DebtItem(
                                                    amount = debt[SessionManager.getCurrentUser()!!.userId]!![key.userId]!!,
 ...
 */
@Composable
fun DebtItem(amount: Double, user: String, user2: String, isClickable: Boolean = false, onClick: () -> Unit = {}) {

  // start and end are the text to display at the start and end of the row
  val money = if (amount >= 0) "${String.format("%.02f", amount)} CHF" else "${String.format("%.02f", -amount)} CHF"
    ...
    Row(horizontalArrangement = Arrangement.SpaceBetween,
        ...
        Box(contentAlignment = Alignment.Center, modifier = Modifier
            ...
            .weight(1f)) {
            if(amount < 0)
                Icon(Icons.AutoMirrored.Filled.ArrowBack, contentDescription = "arrow back", tint = MaterialTheme.colorScheme.primary)
            else
                Icon(Icons.AutoMirrored.Filled.ArrowForward, contentDescription = "arrow forward", tint = MaterialTheme.colorScheme.primary)
        }
...
fun DebtInfo(amount: Double, user: String) {
    ...
    Row(horizontalArrangement = Arrangement.SpaceBetween,
        ...
        Modifier
            .fillMaxWidth()
            // red color for negative amount, green color for positive amount, transparent for 0, pale color
            .background(
                if (amount <= 0) if (amount == 0.0) Color.Transparent else Color(0x80f7a8a8) else Color(0x80a8f7a8)
            )
            .padding(horizontal = 8.dp, vertical = 4.dp)
            .testTag("debt$user")
            .height(IntrinsicSize.Max)) {
...
fun DebtItemPreview() {
    Column {
        DebtItem(amount = 50.0, user = "ZDIAUHDZQUHIZDQHIUZQDHIUZQDHUIZDQHUIZQDHUIZQDHUIZDQHIU", "Bob",true)
...
fun DebtContentTestPreview() {
    ...
    DebtContent(expenses = listOf(expense1, expense2, expense3), users = listOf(User("user001", "Alice", ""), User("user002", "Bob", ""), User("user003", "Charlie", "") ))
}
...
/**
 * Firestore-compatible DTO for a TripNotification, simplifying LocalDateTime to String for compatibility
 * and ensuring that all notification details are correctly handled for interactions with Firestore.
 *
...
data class FirestoreTripNotification(
    ...
    val timestamp: String = "" // LocalDateTime is converted to String to ensure Firestore compatibility
) {
    companion object {
        ...
        fun fromTripNotification(tripNotification: TripNotification): FirestoreTripNotification {
            return FirestoreTripNotification(
                ...
                timestamp = tripNotification.timestamp.format(formatter) // Convert LocalDateTime to String
            )
        }
    }
    ...
    fun toTripNotification(): TripNotification {
        return TripNotification(
            ...
            timestamp = LocalDateTime.parse(this.timestamp, formatter) // Convert String back to LocalDateTime
        )
    }
}
